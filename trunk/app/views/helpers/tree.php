 ...
 **/
	...
	var $helpers = array('Html', 'Seo');

/**
 * TODO
 **/

	function show($name, $data, $extra = '', $url = '', $form = false, $selected = false) {
 ...
 **/
	...
	function flat($name, $data, $selected = null, $legend) {
		list($modelName, $fieldName) = explode('/', $name);
		debug($data);
 ...
 **/
	...
	function _List($data, $extra, $url, $form, $selected, $modelName, $fieldName, $level) {
 ...
 **/
	...
	function _generate_input($data, $modelName, $fieldName, $selected) {
		$checked = '';
 ...
 **/
	...
	function _generate_link($data, $url) {
		if(strstr($url, 'admin')) {
